  ...
  <body>
    ...
    <div class="button-container">
      ...
      <button class="back-btn" onclick="window.location.href='/page2?isLoggedIn=true'">Back</button>
        ...
        <script>
          const serverIP = "%SERVERIP%" // This will be replaced with the server IP

          function refreshData() {
            fetch('http://' + serverIP + '/senddatatotable.php')
            ...
            .then(data => {
              ...
              // Ensure that data.data contains an array of card information
              data.data.forEach(row => {
                  let tr = document.createElement('tr');
                  tr.innerHTML = `<td>${row.card_number}</td><td>${row.added_date}</td><td>${row.balance}</td><td>${row.last_update}</td>`;
...
<style>
    * {
    ...
    .topup-btn, .refund-btn ,.check-btn {
        border: none;
        color: white;
    ...
    .topup-btn:hover, .refund-btn:hover ,.check-btn:hover {
...
<body>
    ...
    <button id="topup-btn" class="topup-btn" onclick="topup()">Topup</button>
    <button id="refund-btn" class="refund-btn" onclick="refund()">Refund</button>
...
<script>
    ...
    function registerUser() {
            ...
            fetch('http://' + serverIP + '/registerUser.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    function topup() {
        ...
        .then(result => {
            ...
            // Ask user for confirmation to top-up
            var confirmTopup = confirm("Are you sure you want to top up " + amount + " to card ID " + cardID + "?");

            if (!confirmTopup) {
                console.log('Top-up canceled.');
                return; // Exit the function
            }
            ...
            // Send POST request to topupCard.php
            fetch('http://' + serverIP + '/topupCard.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json', // Use JSON
                },
            ...
            .then(response => response.json())
            .then(result => {
                // Show result from the server
                console.log(result);
                if (result.success) {
                    alert('Top-up successful! Your new balance is: ' + result.newBalance);
                } else {
                    alert('Error: ' + result.message);
                }
            })
    ...
    function refund() {
        ...
        .then(result => {
            ...
            fetch('http://' + serverIP + '/refundCard.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json', // Use JSON
                },
            ...
            .then(result => {
                ...
                console.log(result);
                if (result.success) {
                    alert('Refund successful! New balance: ' + result.newBalance);
                } else {
                    alert('Error: ' + result.message);
                }
            })
...
void setup() {
  ...
  if (ssid == "" || password == "") {
    wifiManager.startConfigPortal("ESP32-WiFi-Setup");
  } else {
